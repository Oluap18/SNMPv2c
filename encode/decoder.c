...
int Pdus2Pdu (PDUs_t* pdu,PDU_t* t_pdu){
	...
	switch(pdu->present){
		...
		case PDUs_PR_get_bulk_request:
			t_pdu->request_id = pdu->choice.get_bulk_request.request_id;
			t_pdu->variable_bindings = pdu->choice.get_bulk_request.variable_bindings;
			break;
